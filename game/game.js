...
const UPGRADES = [
  ...
  { id:"warmonger",  name:"Warmonger",     desc:"+60% damage, -20% move speed.",        color:'#ff2200', apply:p=>{ p.mods.dmg*=1.60; p.mods.speed*=0.80; } },
  { id:"tactician",  name:"Tactician",     desc:"+30% CDR, +30% range, +15% atk speed.", color:'#00ccff', apply:p=>{ p.mods.cdr*=0.70; p.mods.range*=1.30; p.mods.atkSpd*=1.15; } },
  { id:"lifedrain",  name:"Life Drain",    desc:"+18% lifesteal, +15% damage.",         color:'#cc0055', apply:p=>{ p.mods.lifesteal+=0.18; p.mods.dmg*=1.15; } },
  { id:"fortress",   name:"Fortress",      desc:"+80 max HP, full heal, +50 shield.",   color:'#aaaaff', apply:p=>{ p.hpMax+=80; p.hp=p.hpMax; p.shield=Math.max(p.shield||0,50); if(p.mods.shieldMax<50) p.mods.shieldMax=50; } },
  { id:"overcharge", name:"Overcharge",    desc:"+80% atk speed, +30% ability power.", color:'#ffff00', apply:p=>{ p.mods.atkSpd*=1.80; p.mods.dmg*=1.30; } },
  { id:"blitz",      name:"Blitz Mode",    desc:"+40% speed, +20% damage, +15% atk speed.", color:'#00ffaa', apply:p=>{ p.mods.speed*=1.40; p.mods.dmg*=1.20; p.mods.atkSpd*=1.15; } },
  { id:"soulbound",  name:"Soulbound",     desc:"+15% lifesteal, +30% damage, -25% max HP.", color:'#aa00dd', apply:p=>{ p.mods.lifesteal+=0.15; p.mods.dmg*=1.30; p.hpMax=Math.max(20,Math.floor(p.hpMax*0.75)); p.hp=Math.min(p.hpMax,p.hp); } },
  { id:"overclock2", name:"Turbo Core",    desc:"+40% atk speed, -20% CDR, +20% speed.", color:'#ffcc00', apply:p=>{ p.mods.atkSpd*=1.40; p.mods.cdr*=0.80; p.mods.speed*=1.20; } },
  { id:"shieldwall", name:"Shield Wall",   desc:"+80 max shield that regenerates.", color:'#77aaff', apply:p=>{ p.mods.shieldMax+=80; p.shield=Math.max(p.shield||0, p.mods.shieldMax); } },
  { id:"overdrive",  name:"Overdrive",     desc:"+35% all stats (damage, speed, CDR).", color:'#ff8800', apply:p=>{ p.mods.dmg*=1.35; p.mods.speed*=1.35; p.mods.cdr*=0.75; p.mods.atkSpd*=1.20; } },
  // ===== UNIQUE MECHANIC UPGRADES =====
  { id:"resonance",  name:"Resonance Core", desc:"+20% damage, +15% CDR.",              color:'#bb99ff', apply:p=>{ p.mods.dmg*=1.20; p.mods.cdr*=0.85; } },
  { id:"venomcoat",  name:"Venom Coat",     desc:"+10% lifesteal, +15 dps aura.",        color:'#55ff88', apply:p=>{ p.mods.lifesteal+=0.10; p.mods.aura+=15; } },
  { id:"splitshot",  name:"Split Coil",     desc:"+40% range, +10% atk speed.",          color:'#aaffee', apply:p=>{ p.mods.range*=1.40; p.mods.atkSpd*=1.10; } },
  { id:"ironaura",   name:"Iron Aura",      desc:"+50 max HP, +10 dps aura.",             color:'#ffaa66', apply:p=>{ p.hpMax+=50; p.hp=Math.min(p.hpMax,p.hp+50); p.mods.aura+=10; } },
  { id:"momentum",   name:"Momentum Engine",desc:"+20% damage, +20% move speed.",        color:'#00eeff', apply:p=>{ p.mods.dmg*=1.20; p.mods.speed*=1.20; } },
  { id:"voltcell",   name:"Volt Cell",      desc:"+25% atk speed, +15% damage, +8 dps aura.", color:'#ffff55', apply:p=>{ p.mods.atkSpd*=1.25; p.mods.dmg*=1.15; p.mods.aura+=8; } },
  { id:"deathmark",  name:"Death Mark",     desc:"+40% CDR, +10% lifesteal.",             color:'#cc44ff', apply:p=>{ p.mods.cdr*=0.60; p.mods.lifesteal+=0.10; } },
  { id:"neurallink", name:"Neural Link",    desc:"+15% damage, speed, CDR, and atk speed.", color:'#88ffdd', apply:p=>{ p.mods.dmg*=1.15; p.mods.speed*=1.15; p.mods.cdr*=0.85; p.mods.atkSpd*=1.15; } },
  { id:"assassin",   name:"Assassin Protocol", desc:"+70% damage, -25% max HP.",          color:'#ff2244', apply:p=>{ p.mods.dmg*=1.70; p.hpMax=Math.max(20,Math.floor(p.hpMax*0.75)); p.hp=Math.min(p.hpMax,p.hp); } },
  { id:"overclock3", name:"Neural Overdrive",  desc:"+30% atk speed, +30% speed, -10% max HP.", color:'#22ffcc', apply:p=>{ p.mods.atkSpd*=1.30; p.mods.speed*=1.30; p.hpMax=Math.max(20,Math.floor(p.hpMax*0.90)); p.hp=Math.min(p.hpMax,p.hp); } },
];
